...
from utils.Key import KeyInput
...
class KeyPressThread(QThread):
    ...
    def __init__(self, parentObj):
        ...
        self.key = KeyInput()  # 模拟按键类
        self.mid = self.parentObj.midiProcess  # midi处理类
        self.pool = multiprocessing.Pool(10)  # 进程池，最大进程数量
        self.index = 0  # 当前下标
        ...
        while True:
            ...
            if not self.pause:
                ...
                for item in self.mid.event_dict[tick].values():
                    ...
                    if self.parentObj.mode == 1:
                        ...
                        if item['type'] == 'note_on':
                            # print('模式一按压')
                            self.pool.apply_async(self.key.ys_press_key, (item['val'],))
                        elif item['type'] == 'note_off':
                            # print('模式一松开')
                            self.pool.apply_async(self.key.ys_relase_key, (item['val'],))
                    elif self.parentObj.mode == 2:
                        # 模式二
                        # print('模式二按压')
                        self.pool.apply_async(self.key.ys_press_key_mode2, (item['val'],))
                # 休眠
                ...
                self.playTimesSingal.emit(
                    (100.0 * self.mid.event_tick[self.index]) / self.mid.event_tick[-1]
                )  # 发射进度
                self.index += 1
